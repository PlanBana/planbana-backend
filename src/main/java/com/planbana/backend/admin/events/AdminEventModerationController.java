...
public class AdminEventModerationController {
        ...
        // ============================================================
        // 1️⃣2️⃣ UNASSIGN MODERATOR
        // ============================================================
        @PostMapping("/{eventId}/unassign")
        @PreAuthorize("hasRole('ADMIN')")
        public Map<String, Object> unassignModerator(
                        @PathVariable String eventId,
                        Authentication auth) {
                Event event = getEvent(eventId);
                String adminId = getAdminId(auth);

                if (event.getAssignedModerator() == null) {
                        return Map.of("status", "already_unassigned");
                }

                String oldModerator = event.getAssignedModerator();
                event.setAssignedModerator(null);
                eventRepo.save(event);

                auditLogger.log(
                                AuditCategory.ADMIN_EVENTS,
                                AuditAction.ADMIN_UNASSIGNED_EVENT,
                                adminId,
                                event.getCreatedByUserId(),
                                eventId,
                                Map.of("oldModerator", oldModerator));

                activityFeed.publish(Map.of(
                                "type", "EVENT_UNASSIGNED",
                                "eventId", eventId,
                                "adminId", adminId,
                                "oldModerator", oldModerator));
